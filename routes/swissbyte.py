...
def evaluate_code(challenge):
    ...
    def check_condition(line, variables):
        var = variables[line[1]] if line[1] in variables else int(line[1])
        condition = line[2]
        comparison = variables[line[3]] if line[3] in variables else int(line[3])

        return ((condition == '==' and var != comparison) or 
                (condition == '!=' and var == comparison) or 
                (condition == '>' and var <= comparison) or 
                (condition == '<' and var >= comparison))

    def execute_assignment(line, variables):
        if len(line) == 3:
            variables[line[0]] = variables[line[2]] if line[2] in variables else int(line[2])
        else:
            var1 = variables[line[2]] if line[2] in variables else int(line[2])
    ...
    def execute_code(code, variables):
        ...
        while i < len(code):
            ...
            if line[0] == 'fail':
                ...
                if check_condition(line, variables):
                    while code[i] != 'endif':
                        i += 1
                else: # if the condition is true, we execute the code inside the if block
                    new_code = []
                    i += 1
                    while code[i] != 'endif':
                        new_code.append(code[i])
                        i += 1
                    is_solvable, variables = execute_code(new_code, variables)
                    if is_solvable == "false":
                        return "false", variables

            elif line[1] == '=':
                variables = execute_assignment(line, variables)
